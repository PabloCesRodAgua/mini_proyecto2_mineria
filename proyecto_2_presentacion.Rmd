...
Note: For reasons of the Data Mining class and less complexity, we will be using "Min Max Normalization". Also is its important to say that this two distances are not the only ones avaiable, there are also distance functions like Manhatan.. (Pendiente) 
...
students_dataset_norm <- as.data.frame(lapply(data[,c(1:17)], minmax_normalization))
```
